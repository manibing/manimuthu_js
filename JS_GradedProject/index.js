...
let resumes=JSON.parse(jsonstr);
//console.log(jsonstr);
//console.log(resumes);
//console.log("Resumess instanceof Object: "+(resumes instanceof Object));
//console.log(resumes.resume[0].work['Summary']);

function resumebasics(index){
  var name=resumes.resume[index].basics.name;
  var AppliedFor=resumes.resume[index].basics.AppliedFor;
  var image=resumes.resume[index].image;
  var email=resumes.resume[index];
  console.log(name);
  console.log(email);
...
// var experience=document.getElementById('workExp');
...
// 	 resumeList=[experience,projects,education,internship,achievements,personalInfo,skills,hobbies];

function itemDisplay(){
	var items=resumes.forEach((ele,index)=>{
		ele.headerSection(index);
     return items;
	})
}
itemDisplay();
...
function headerSection(index){
	var name=resumes.resume[index].basics.name;
	
	var AppliedFor=resumes.resume[index].basics.AppliedFor;
	var headerlist=[name,AppliedFor];
	 return headerlist;
}
console.log(headerSection(0));

function Experience(index){
	var companyName=resumes.resume[index].work['Company Name'];

	var position=resumes.resume[index].work['Position'];
	var startDate=resumes.resume[index].work['Start Date'];
	var endDate=resumes.resume[index].work['End Date'];
	var summary=resumes.resume[index].work['Summary'];
	 var expList=[companyName,position,startDate,endDate,summary];
...
function Projects(index){
	var projectName=resumes.resume[index].projects.name;
	var description=resumes.resume[index].projects.description;

	var details=[projectName,description]
	return details;
}
...
function display(){
	...
	var projects=document.getElementById('projects');
	 ...
	 var workdata=[];
	 var expdatas=resumes.forEach((ele,index) => {
		 let companyname=ele.companyname[index];
		 let position=ele.position[index];
		 let startdate=ele.startdate[index];
		 let enddate=ele.enddate[index];
		 let summary=ele.summary[index];
		 var newworkdata=[companyname,position,startdate,enddate,summary];
